...
class ShlokDetailScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Granular watches — each widget rebuilds only on the field it needs.
    final rs = ref.watch(readingStylesProvider);
    final showTransliteration = ref.watch(showTransliterationProvider);
    final showCommentary = ref.watch(showCommentaryProvider);
      ...
      data: (shlok) => EditorialLayout(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            ...
            SliverToBoxAdapter(
              child: _VerseTranslation(shlok: shlok, rs: rs),
            ),
            // Commentary — gated on both domain data and user setting
            if (shlok.hasCommentary && showCommentary)
              SliverToBoxAdapter(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            // titleLarge is Inter-free NotoSerif; not scaled (header/metadata)
...
// ─────────────────────────────────────────────────────────────────────────────
// Sanskrit section — consumes ReadingStyles; gated transliteration
// ─────────────────────────────────────────────────────────────────────────────
      ...
      child: Column(
        ...
        children: [
          SectionContainer(
            ...
            child: Text(
              shlok.sanskritText,
              // Consume pre-scaled style from ReadingStyles — no math here.
              style: rs.sanskritDisplay.copyWith(
          ...
          // Transliteration — gated on both content and user preference.
          // AnimatedSize with design-system duration for smooth collapse.
          AnimatedSize(
            duration: AppAnimations.standard,
            curve: AppAnimations.defaultCurve,
            child: shlok.transliteration.isNotEmpty && showTransliteration
                ? Padding(
                    padding: const EdgeInsets.only(top: AppSpacing.lg),
                    child: Text(
                      shlok.transliteration,
                      style: rs.bodyMedium.copyWith(
...
// ─────────────────────────────────────────────────────────────────────────────
// Translation — consumes ReadingStyles
// ─────────────────────────────────────────────────────────────────────────────

class _VerseTranslation extends StatelessWidget {
  const _VerseTranslation({required this.shlok, required this.rs});
  final Shlok shlok;
  final ReadingStyles rs;
...
// ─────────────────────────────────────────────────────────────────────────────
// Commentary — consumes ReadingStyles; visibility gated at parent
// ─────────────────────────────────────────────────────────────────────────────
