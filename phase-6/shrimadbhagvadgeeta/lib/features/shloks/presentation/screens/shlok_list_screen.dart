...
class _ShlokIndexRow extends ConsumerStatefulWidget {
  const _ShlokIndexRow({required this.shlok, required this.onTap});
  final Shlok shlok;
  final VoidCallback onTap;

  @override
  ConsumerState<_ShlokIndexRow> createState() => _ShlokIndexRowState();
}

class _ShlokIndexRowState extends ConsumerState<_ShlokIndexRow>
        ...
        child: SectionContainer(
          ...
          child: Row(
            children: [
              ...
              // Consume pre-scaled reading styles — no fontSize math here.
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    // Sanskrit first line — NotoSerifDevanagari, scaled
                    Consumer(
                      builder: (_, ref, __) {
                        final rs = ref.watch(readingStylesProvider);
                        return Text(
                          _firstLine(widget.shlok.sanskritText),
                          style: rs.sanskritSmall.copyWith(
                            color: scheme.onSurface,
                          ),
                          maxLines: 1,
                    ...
                    // Translation preview — NotoSerif, scaled
                    Consumer(
                      builder: (_, ref, __) {
                        final rs = ref.watch(readingStylesProvider);
                        return Text(
